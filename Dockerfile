...
RUN cd /tmp && wget https://www.hepforge.org/archive/yoda/YODA-$YODA_VERSION.tar.gz && tar -xzf YODA-$YODA_VERSION.tar.gz && cd YODA-$YODA_VERSION && ./configure && make -j4 && sudo make install && cd ..
